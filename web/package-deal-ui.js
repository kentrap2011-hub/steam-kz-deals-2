window.renderPackageDeal=function(g){
  ...
  if(!sourceAligned){
    rankNote='Набор подтверждён по составу. Сравнение выгоды и влияние на рейтинг обновятся после синхронизации цен.';
  }else if(drivesRank){
  ...
  const economics=[];
  if(sourceAligned&&standalone!=null)economics.push(`по отдельности ${fmtRub(standalone)}`);
  ...
  if(sourceAligned&&!strict&&Number.isFinite(delta)&&delta>0)economics.push(`на ${fmtRub(delta)} дороже этих игр отдельно`);
  if(sourceAligned&&!strict&&Number.isFinite(delta)&&delta===0)economics.push('по той же цене, что и эти игры отдельно');
