...
- Story-DLC semantic scope gate: `config/mailing_policy.json` → `scripts/story_dlc_scope.py` → `scripts/build_pre_ai_family_graph.py` → `scripts/build_pre_ai_chatgpt_payload.py`; dossier producer `scripts/taste_steam_review_dossier_web.py` repeats a fail-closed guard. Positive eligibility comes from reliable product metadata, while non-story/unproven DLC is removed before dossier work. The family graph retains compact machine-readable classification/audit summary; exact package/member purchase identity remains separate.
