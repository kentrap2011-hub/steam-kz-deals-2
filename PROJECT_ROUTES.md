...
- **Story-DLC scope gate:** `scripts/build_pre_ai_chatgpt_payload.py` применяет `scripts/taste_steam_review_dossier_web.py::classify_story_dlc_scope` к independent add-on semantic candidates **до** публикации `chatgpt_taste_queue.jsonl`; non-story/unproven DLC поэтому не создаёт отдельную Taste semantic obligation. Dossier identity projection повторяет тот же classifier defense-in-depth для любого add-on row, который всё же до него дошёл. Positive story evidence берётся из canonical Steam `short_description`; отсутствие positive proof исключает fail-closed.
- Полный upstream audit живёт в `chatgpt_payload.json.story_dlc_scope`; dossier snapshot сохраняет собственный defense-in-depth `taste_steam_review_dossier_work.json.story_dlc_scope`. Story policy revision входит в same-day snapshot compatibility guard через `scripts/build_taste_steam_review_dossier_work.py`; `appid 2378500` является обязательной non-story regression.
