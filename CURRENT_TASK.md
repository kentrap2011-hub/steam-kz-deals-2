...
### Taste dossier transient author dedupe fallback implement 01
Статус: `in_progress`.
- worker task: `WORKER_TASK_TASTE_DOSSIER_TRANSIENT_AUTHOR_DEDUPE_FALLBACK_IMPLEMENT_01.md`;
- scope: privacy-preserving transient-author dedupe fallback only when a concrete feedback item is visible but no neutral stable item locator is obtainable;
- neutral stable locator remains preferred; exact-product identity and story-DLC scope remain unchanged;
- no Scheduled Task `Run now`; no new queue/retry/stage.
