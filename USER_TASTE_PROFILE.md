...
- Current risk labels such as `management_routine`, `puzzle_pacing` and similar model outputs remain hypotheses until checked against concrete user comparisons. `directionlessness` now has direct user evidence only as a contextual concern, while `unchanged_repetition` now has direct evidence **against** treating repetition itself as a standalone dislike.
