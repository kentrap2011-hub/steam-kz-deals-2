...
- **Organic discovery is a positive structural signal.** Dmitry explicitly says that finding things by chance is enjoyable. If all interesting content is revealed in advance, unexplored areas lose some attraction because he already knows whether anything is there. Therefore a fully pre-marked checklist/map can reduce exploration motivation even when the underlying content remains good. Do not over-generalize this into a dislike of all map markers; the confirmed point is that uncertainty gives exploration value.
...
- **Positive challenge signal:** demanding mastery challenges can increase motivation when they feel like a personal skill test rather than tedious system study or grind. `Sifu`'s no-aging achievement is attractive specifically because Dmitry wants to test himself. This contrasts with `HighFleet`, where complexity feels instructional/technical rather than like an appealing mastery challenge.
...
- Current risk labels such as `unchanged_repetition`, `management_routine`, `puzzle_pacing` and similar model outputs remain hypotheses until checked against concrete user comparisons. `directionlessness` now has direct user evidence, but only as a contextual concern rather than a universal negative.
...
- How much map guidance is comfortable before it starts undermining Dmitry's exploration curiosity? Current evidence says full pre-revelation is a negative, but it does not establish whether light hints/partial markers are also undesirable.
...
- Which kinds of achievements increase interest most: mastery/skill challenges, exploration, completion, unusual play constraints, or other forms? Existing evidence strongly supports mastery challenge and general replay motivation but not every achievement type.
