...
- Current risk labels such as `puzzle_pacing` and similar model outputs remain hypotheses until checked against concrete user comparisons. `directionlessness` is a contextual concern; `unchanged_repetition` has evidence against repetition itself being the problem; and `management_routine` now points specifically toward **mandatory upkeep over time** rather than management mechanics in general.
