...
### Player-feedback acquisition modes, optional item locators and non-authoritative `mention_count`

Evidence usability is primarily about **what concrete player feedback was actually observed** and whether it can be bound confidently to the exact target product. A permanent identity for every individual review is not required.

Persist one source-level `acquisition_mode` for every provenance source:

- `context_only` — official/professional/product metadata; never a player-feedback mention;
- `stable_item` — concrete player feedback with a safe non-identifying stable item locator;
- `inspected_collection_item` — concrete player-feedback content directly inspected on an exact-product collection/page/card surface, even when no individual item locator is available;
- `search_result_observation` — concrete player-feedback content directly inspected in a search/discovery result representation, even when opening the target page later fails.

Use these rules:

1. **Stable item when readily available.** A safe neutral recommendation/review/comment/post locator remains preferred auditability metadata. Serialize it as `identity_mode:"stable_locator"` with the next dossier-local `feedback-NNN` id and keep all exact parent/child/appid checks.
2. **No locator or author identity is required.** If concrete player-feedback content is directly visible on an exact-product collection/card representation or search/discovery representation, serialize one `identity_mode:"source_observation"` support record with the next dossier-local `observation-NNN` id. Omit item `url` and `public_ref`; the safe parent/result provenance carries where and how the information was observed.
3. A source-observation parent uses `feedback_surface_mode:"concrete_item_collection"` with `acquisition_mode:"inspected_collection_item"`, or `feedback_surface_mode:"search_result_representation"` with `acquisition_mode:"search_result_observation"`.
4. Every locatorless source-observation parent must persist `exact_product_binding` with exact `appid`, exact descriptor `title`, resolved `release_year`, and a non-query-only `binding_basis` from the machine contract. The binding must match the dossier exactly. Search query text, a target name merely appearing in the query, a domain hit, locale parameter, or generic site result is never sufficient binding.
5. A search/discovery representation is usable only when the worker actually received and inspected **concrete player-authored/player-feedback content** in that representation. Aggregate counts, ratings, percentages, language totals, publisher/editorial copy, and a result that merely links to a page are not concrete player feedback.
6. If usable concrete content was already inspected in a search/discovery representation, later failure to open/read the target page does not erase that evidence. Record only the safe result/source provenance and `search_result_observation`; do not persist the failed transport body, raw snippet, or any quoted review text.
7. **Never require or chase author identity for evidence validity.** Usernames, display names, Steam/account/profile ids, profile URLs, hashes or predictable pseudonyms are neither needed nor persisted. The older `transient_author_deduped` serialization remains validator-compatible for historical/migration compatibility, but the active worker must not make usable evidence depend on it.
8. Avoid obvious duplicate inflation. The same or clearly equivalent surfaced content/source representation must not be serialized repeatedly merely because it appears through equivalent query/locale/route variants. A locatorless `source_observation` is one support record for that physical source representation; do not manufacture several pseudo-items when individual identity is unavailable.
9. Multiple materially independent source observations may justify stronger qualitative recurrence. Do not claim an exact review population unless an allowed aggregate source explicitly supplies that population for a permitted aggregate claim; aggregate populations never become individual feedback records.

Every observation/conflict still lists its compact supporting `player_feedback_ids`. `mention_count` remains only a backward-compatible integer equal to the number of dossier-local bound support records. It is **not** an exact count of reviews in the population, is not a completeness threshold, and does not determine recurrence.

Recurrence is qualitative and evidence-grounded:

- `anecdotal` — isolated/single observed support;
- `limited` — some support, but weak, narrow or not independently broad enough for a stronger label;
- `moderate` — a materially corroborated recurring pattern across the inspected evidence;
- `strong` — broad/repeated materially independent support sufficient to justify the strongest recurrence label.

There is no hidden requirement for 2/3/5 records and no `moderate`/`strong` minimum number of stable item locators. A stable locator improves auditability; its absence alone never invalidates otherwise usable exact-product observed feedback or caps the whole Dossier.
...
- `found_and_used` — at least one Russian- or mixed-language **concrete player-feedback support record was actually observed and used** in an observation/conflict through any allowed evidence mode: `stable_locator`, exact-product `inspected_collection_item`, or exact-product `search_result_observation`. A per-item locator and author identity are not required.
- `searched_no_existence_signal` — a bounded good-faith Russian search was completed and did not establish reliable exact-product Russian player-feedback activity or expose usable Russian/mixed concrete content; this is a valid terminal state for an otherwise sufficient dossier.
- `existence_established_retrieval_unresolved` — reliable exact-product Russian player-feedback activity was established, but after required materially distinct routes **no usable concrete Russian/mixed player-feedback content was actually observed**. Missing per-item locator alone is never this state.
- `existence_established_access_unresolved` — reliable exact-product Russian player-feedback activity was established, but access/tooling prevented the worker from actually seeing usable concrete Russian/mixed player-feedback content. If a search/discovery representation already exposed usable concrete content, a later direct-open/read failure is not this state.

Only `found_and_used` and `searched_no_existence_signal` are complete-dossier states. The two `existence_established_*_unresolved` states remain genuine content-unavailable/access-unresolved failures; do not publish that game or the three-game group when one of those states genuinely applies.

A reliable existence signal about the exact product/appid remains discovery metadata only when it exposes only aggregate activity such as a nonzero Russian-language review population. Aggregate activity, a Russian-rendered UI, a query string, a domain hit or professional Russian content is not concrete player feedback and cannot itself satisfy `found_and_used`.

The relationship with used evidence remains bidirectional: if any Russian/mixed support record is actually bound and used, `russian_attempt` must be `found_and_used`; no other state can coexist with used Russian/mixed evidence.
...
Use semantic/adaptive retrieval, preserving TASTE-014:

1. Prefer a cheap exact-product source that already exposes usable concrete Russian/mixed player feedback.
2. If the first route yields only aggregate/list/index data, inaccessible content, non-Russian concrete cards, or otherwise no usable concrete feedback, pivot to a materially distinct promising player-feedback surface when one is reasonably discoverable.
3. A search/discovery result representation is itself a valid observation surface when it visibly exposes concrete player-authored feedback and carries enough source/result metadata to establish the exact target product. Do **not** require opening the target page merely to make already-observed content usable.
4. If the representation exposes a safe stable item locator, keep it as preferred auditability metadata. If it does not, use `source_observation`; do not chase author/profile identity and do not spend materially equivalent retries merely to manufacture a per-item locator.
5. Continue materially distinct routes as required by coverage, temporal completeness and exact-product safety. Finding one Russian item closes the Russian retrieval gate only; it does not automatically make the overall Dossier sufficient.

Exact-product safeguards remain strict. Base-game feedback cannot satisfy an exact DLC/edition dossier. Original-release feedback cannot silently satisfy a remake/remaster when release identity matters. Sequel/prequel/same-named unrelated results and results whose actual URL/metadata bind another product fail closed even if the query text names the target.

### Search/discovery representation safety

For `search_result_observation`:

- the worker must actually inspect concrete player-feedback content in the returned result representation;
- persist only the safe source/result locator, acquisition mode, exact-product binding, language/freshness/source classification, and neutral paraphrased observation;
- never persist the raw snippet, quote, username, author/profile identity or reversible identity token;
- `exact_product_binding` must match the dossier `appid`, exact descriptor title and resolved release year using result/source metadata or another permitted non-query-only basis;
- a query that mentions the target while the result URL/content belongs to another appid/product is invalid;
- a generic site/domain result with no exact-product binding is invalid;
- a transport failure such as an HTTP/tool error after the concrete result representation was already inspected does not invalidate that already-observed evidence.

### Exact-product collection/card observation

For `inspected_collection_item`:

- the worker must directly inspect at least one concrete player-feedback card/item on the exact-product collection/page;
- the source must carry safe exact-product parent provenance and the required `exact_product_binding`;
- the item need not have a stable public URL/ref and the worker need not identify its author;
- serialize a dossier-local `source_observation` support record and a neutral paraphrase;
- do not claim independently reopenable per-item identity unless a real stable locator exists.

An exact-app Steam Store page may still serve as a concrete collection parent when individual review cards are actually visible, with the existing review-surface source-type restriction. Its aggregate counts/ratings/language totals remain non-feedback metadata.
