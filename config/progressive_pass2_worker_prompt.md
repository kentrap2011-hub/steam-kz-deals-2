...
6. Immediately before starting semantic execution for an item, verify that the canonical dossier still has the exact prepared `dossier_content_sha256`, exact prepared `dossier_compatibility_binding`, and exact `expires_at_utc` copied into `dossier_expires_at_utc`, and that this expiry is strictly later than current UTC. If any liveness check fails, publish no result or terminal receipt for that item, consume no attempt, and do not rebuild or reinterpret scope.\n8. Do not use price, discount, sale urgency, wishlist, purchase value, or other commercial signals for the semantic judgment.
