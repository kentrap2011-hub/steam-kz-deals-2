...
def _validate_no_author_identity_payload(dossier, evidence_contract):
    """Author/profile identity is never persisted in any pragmatic evidence mode."""
...
def _validate_source_exact_product_binding(source, label, enums, *, exact_appid, expected_title, expected_release_year):
    binding = source.get("exact_product_binding")
    if not isinstance(binding, dict):
        raise ValueError(f"{label}.exact_product_binding is required for locatorless observed feedback")
    required = {"appid", "title", "release_year", "binding_basis"}
    if set(binding) != required:
        raise ValueError(f"{label}.exact_product_binding must contain exactly appid, title, release_year and binding_basis")
    if binding.get("appid") != str(exact_appid):
        raise ValueError(f"{label}.exact_product_binding appid does not match exact dossier appid")
    if binding.get("title") != expected_title:
        raise ValueError(f"{label}.exact_product_binding title does not match exact dossier title")
    if not _json_int(binding.get("release_year")) or binding.get("release_year") != expected_release_year:
        raise ValueError(f"{label}.exact_product_binding release_year does not match exact dossier release")
    if binding.get("binding_basis") not in enums["exact_product_binding_basis"]:
        raise ValueError(f"{label}.exact_product_binding binding_basis is invalid or query-only")


def _validate_source(source, index, enums, schema_doc, generated_date, evidence_contract, *, exact_appid=None, expected_title=None, expected_release_year=None):
    ...
    if acquisition_mode not in enums["source_acquisition_mode"]:
        raise ValueError(f"{label}.acquisition_mode is invalid")
    if acquisition_mode == "context_only":
        if source["player_feedback"] is not False:
            raise ValueError(f"{label} context_only acquisition cannot be player feedback")
        if feedback_surface_mode is not None:
            raise ValueError(f"{label} context_only acquisition cannot carry a feedback surface mode")
    elif source["player_feedback"] is not True:
        raise ValueError(f"{label} player-feedback acquisition mode requires player_feedback=true")

    if acquisition_mode == "inspected_collection_item":
        if feedback_surface_mode != "concrete_item_collection":
            raise ValueError(f"{label} inspected_collection_item requires concrete_item_collection surface mode")
        _validate_source_exact_product_binding(
            source, label, enums,
            exact_appid=exact_appid,
            expected_title=expected_title,
            expected_release_year=expected_release_year,
        )
    elif acquisition_mode == "search_result_observation":
        if feedback_surface_mode != "search_result_representation":
            raise ValueError(f"{label} search_result_observation requires search_result_representation surface mode")
        _validate_source_exact_product_binding(
            source, label, enums,
            exact_appid=exact_appid,
            expected_title=expected_title,
    ...
    elif source.get("exact_product_binding") is not None:
        _validate_source_exact_product_binding(
            source, label, enums,
            exact_appid=exact_appid,
            expected_title=expected_title,
    ...
    if source_type in context_types:
        ...
        if acquisition_mode != "context_only":
            raise ValueError(f"{label} context-only source type requires context_only acquisition mode")
    if _is_steam_store_app_page(source) and (source["player_feedback"] is True or source_type in player_types):
        if acquisition_mode not in {"inspected_collection_item", "search_result_observation"}:
            raise ValueError(
                f"{label} Steam Store app page is not itself a player-feedback item; "
                "it may only parent inspected concrete cards or an inspected search-result representation"
            )
        expected_surface = (
            "concrete_item_collection"
            if acquisition_mode == "inspected_collection_item"
            else "search_result_representation"
        )
        if feedback_surface_mode != expected_surface:
            raise ValueError(f"{label} Steam Store observed-feedback surface mode does not match acquisition mode")
        if source_type not in {"steam_reviews", "store_user_reviews"}:
            raise ValueError(
                f"{label} Steam Store observed-feedback parent must use a review-surface source type"
            )
...
def _validate_feedback_record(record, index, source_map, enums, schema_doc, generated_date, evidence_contract, *, exact_appid=None):
    ...
    if identity_mode == "stable_locator":
        ...
        if "url" in record or "public_ref" in record:
            raise ValueError(f"{label} source_observation must not persist an item url or public_ref")
        if not _SOURCE_OBSERVATION_FEEDBACK_ID_RE.fullmatch(feedback_id):
            raise ValueError(f"{label} source_observation feedback_id must be dossier-local observation-NNN")
        if source.get("acquisition_mode") not in {"inspected_collection_item", "search_result_observation"}:
            raise ValueError(f"{label} source_observation requires an allowed observed-feedback acquisition mode")
        expected_surface = (
            "concrete_item_collection"
            if source.get("acquisition_mode") == "inspected_collection_item"
            else "search_result_representation"
        )
        if source.get("feedback_surface_mode") != expected_surface:
            raise ValueError(f"{label} source_observation parent surface mode does not match acquisition mode")
        if isinstance(source.get("url"), str) and _is_profile_scoped_url(source["url"], evidence_contract):
            raise ValueError(f"{label} source_observation parent URL must not be author/profile-scoped")
        item_identity = f"source-observation:{_source_locator_identity(source)}"
    else:
        if "url" in record or "public_ref" in record:
        ...
        if source.get("acquisition_mode") != "inspected_collection_item":
            raise ValueError(f"{label} transient-author fallback requires inspected_collection_item acquisition mode")
...
def _validate_observations(dossier, source_map, feedback_map, enums, schema_doc):
    ...
    for index, observation in enumerate(observations):
        ...
        if not _json_int(mention_count) or mention_count != len(feedback_ids):
            raise ValueError(f"observation {index} mention_count must equal bound dossier-local support records")
...
def _validate_conflicts(dossier, source_map, feedback_map, enums, schema_doc):
    ...
    for index, conflict in enumerate(conflicts):
        ...
        if not _json_int(mention_count) or mention_count != len(feedback_ids):
            raise ValueError(f"conflict {index} mention_count must equal bound dossier-local support records")
    ...
    *,
    ...
    for index, source in enumerate(sources):
        ...
        source_id, ref_identity = _validate_source(
            ...
            evidence_contract,
            exact_appid=appid,
            expected_title=dossier.get("title"),
            expected_release_year=(dossier.get("game_identity") or {}).get("release_year"),
        )
    ...
    source_observation_ids = [
        record["feedback_id"]
        for record in feedback_records
        if _feedback_identity_mode(record) == "source_observation"
    ]
    expected_source_observation_ids = [
        f"observation-{index:03d}" for index in range(1, len(source_observation_ids) + 1)
    ]
    if source_observation_ids != expected_source_observation_ids:
        raise ValueError("source-observation feedback ids must be sequential dossier-local observation-NNN tokens")
    ...
    _validate_no_author_identity_payload(dossier, evidence_contract)
    ...
    if russian_attempt == "existence_established_retrieval_unresolved":
        raise ValueError(
            "Russian exact-product activity is established but usable concrete player-feedback content was not observed"
        )
    if russian_attempt == "existence_established_access_unresolved":
        raise ValueError(
            "Russian exact-product activity is established but access prevents observing usable concrete player-feedback content"
        )
