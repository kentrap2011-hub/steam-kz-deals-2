...
def run():
    ...
    for item in items:
        try:
            ...
            key = item['key']
            failures.record_game(key, appid=item.get('appid'), name=item.get('title'), stage='review_candidate_selection', error=exc, prior_site_data_exists=key in previous_by_key)
            failed_keys.add(key)

    review_appids = sorted({str(item['appid']) for item in review_candidate_items if item.get('appid') and str(item['appid']).isdigit()})
    review_item_keys = {}
    ...
    if review_appids:
        with ThreadPoolExecutor(max_workers=core['REVIEW_WORKERS']) as executor:
            futures = {executor.submit(core['get_review_pair'], appid): appid for appid in review_appids}
            for future in as_completed(futures):
                ...
                try:
                    ...
                    error = None
                if result is None or not result.get('global', {}).get('ok') or not result.get('russian', {}).get('ok'):
                    if result:
                    ...
                    for key in review_item_keys.get(appid, []):
                        ...
                        failures.record_game(key, appid=appid, name=item.get('title'), stage='review_enrichment', error=error or RuntimeError('Steam Reviews API did not return both required summaries'), prior_site_data_exists=key in previous_by_key)
                        failed_keys.add(key)
                    continue
    ...
    for item in items:
        ...
        try:
            ...
            failures.record_game(key, appid=item.get('appid'), name=item.get('title'), stage='broad_selection', error=exc, prior_site_data_exists=key in previous_by_key)
            failed_keys.add(key)
            continue
        if not reasons:
        ...
        broad.append({
            'key': key, 'appid': item['appid'], 'title': item['title'],
            'discount_percent': item['discount_percent'], 'final_kzt': item['final_kzt'],
            'review_positive': item['global_review_positive'], 'review_count': item['global_review_count'],
            'global_review_positive': item['global_review_positive'], 'global_review_count': item['global_review_count'],
            'russian_review_positive': item['russian_review_positive'], 'russian_review_count': item['russian_review_count'],
            'fit_tags': fit_tags, 'release_date': item['release_date'], 'broad_reasons': reasons,
        })
    ...
    for item in broad:
        ...
        try:
            ...
            failures.record_game(key, appid=item.get('appid'), name=item.get('title'), stage='refined_selection', error=exc, prior_site_data_exists=key in previous_by_key)
            failed_keys.add(key)
            continue
        successful_keys.add(key)
        if not reasons:
        ...
        selected.append({
            'key': key, 'appid': item['appid'], 'discount_percent': item['discount_percent'], 'final_kzt': item['final_kzt'],
            'review_positive': item['global_review_positive'], 'review_count': item['global_review_count'],
            'global_review_positive': item['global_review_positive'], 'global_review_count': item['global_review_count'],
            'russian_review_positive': item['russian_review_positive'], 'russian_review_count': item['russian_review_count'],
            'core_fit_count': core_fit_count, 'reasons': reasons, 'fit_tags': item['fit_tags'],
            'release_date': item['release_date'], 'title': item['title'],
        })

    failures.resolve_games(successful_keys)
    selected, preserved_keys = preserve_last_known_good(selected, previous_by_key, failed_keys)
    ...
    selected.sort(key=lambda item: (-len(item.get('reasons') or []), -int(item.get('core_fit_count') or 0), -int(item.get('discount_percent') or 0), float(item.get('final_kzt') or 0), str(item.get('title') or '').casefold()))
    ...
    if search_review_coverage < 0.40:
        failures.write()
        raise SystemExit(f'Steam search review parsing coverage too low: {search_review_coverage:.3f}')
    if not selected:
        failures.write()
        raise SystemExit('Production shortlist is empty')

    columns = previous_columns or [
        'key', 'appid', 'discount_percent', 'final_kzt', 'review_positive', 'review_count',
        'global_review_positive', 'global_review_count', 'russian_review_positive', 'russian_review_count',
        'core_fit_count', 'reasons', 'fit_tags', 'release_date', 'title',
    ]
    required_columns = ['key', 'appid', 'discount_percent', 'final_kzt', 'review_positive', 'review_count', 'global_review_positive', 'global_review_count', 'russian_review_positive', 'russian_review_count', 'core_fit_count', 'reasons', 'fit_tags', 'release_date', 'title']
    ...
    manifest = {
        'collector_version': 8,
        'source': 'Steam Store', 'country_code': 'kz', 'region': 'Kazakhstan',
        'started_at_utc': started.isoformat(), 'updated_at_utc': finished.isoformat(),
        'page_size': core['PAGE_SIZE'], 'steam_total_reported': reported_total, 'unique_items': len(items),
        'rows_seen': traversal['rows_seen'], 'duplicate_rows_seen': traversal['duplicate_rows'],
        'requests_made': traversal['requests_made'], 'recovery_pass_used': False, 'traversal_pass_count': 1,
        'coverage_ratio': round(coverage, 6) if coverage is not None else None,
        'complete': True,
        'catalog_count_drift_informational': count_drift,
        'source_has_known_gaps': bool(failures.failed_segment_ids_this_run),
        ...
        'review_candidate_items': len(review_candidate_items), 'review_candidate_appids': len(review_appids),
        'review_api_requests': logical_review_requests, 'review_api_failed_requests': review_api_failed_requests,
        'review_api_failure_rate': round(review_api_failed_requests / logical_review_requests, 6) if logical_review_requests else 0,
        'global_review_appids_ok': len(review_cache), 'russian_review_appids_ok': len(review_cache),
        'review_selection_rule': 'global_count_and_global_or_russian_rating', 'review_count_basis': 'global',
        'russian_review_count_cap': core['RUSSIAN_REVIEW_COUNT_CAP'], 'russian_rating_comparison': 'steam_display_whole_percent',
        'review_policy_regression_guard': True, 'review_threshold_profile': core['REVIEW_THRESHOLD_PROFILE'],
        'review_thresholds': dict(core['REVIEW_THRESHOLDS']), 'global_review_language': 'all',
        'russian_review_language': 'russian', 'review_purchase_type': 'all',
        'items_with_tags': items_with_tags, 'tag_coverage': round(tag_coverage, 6),
        'excluded_obvious_extras': excluded_extra, 'excluded_software_only': excluded_software,
        'broad_shortlist_items': len(broad), 'shortlist_items': len(selected), 'shortlist_chunk_size': core['SHORT_CHUNK'],
        'shortlist_chunk_count': chunk_count, 'free_items': previous_manifest.get('free_items'),
        'needs_tuning': len(selected) > 800, 'broad_reason_counts': dict(broad_reason_counts),
        'reason_counts': dict(refined_reason_counts),
        'partial_publish_failure_queue': 'data/cache/steam_partial_publish_failures.json',
    ...
    index = {
        'version': 8, 'format': 'tsv', 'columns': columns, 'country_code': 'kz', 'item_count': len(selected),
        'chunk_size': core['SHORT_CHUNK'], 'chunk_count': chunk_count,
        'chunk_pattern': 'data/production/shortlist/chunk_NNN.tsv',
        'source_total': reported_total, 'source_complete': True,
        'source_coverage_ratio': round(coverage, 6) if coverage is not None else None,
        'source_has_known_gaps': bool(failures.failed_segment_ids_this_run),
        'source_updated_at_utc': finished.isoformat(), 'needs_tuning': len(selected) > 800,
        'review_selection_rule': 'global_count_and_global_or_russian_rating', 'review_count_basis': 'global',
        'russian_review_count_cap': core['RUSSIAN_REVIEW_COUNT_CAP'], 'russian_rating_comparison': 'steam_display_whole_percent',
        'review_policy_regression_guard': True, 'review_threshold_profile': core['REVIEW_THRESHOLD_PROFILE'],
        'review_thresholds': dict(core['REVIEW_THRESHOLDS']),
        'review_fields': {'global': ['global_review_positive', 'global_review_count'], 'russian': ['russian_review_positive', 'russian_review_count']},
        'reason_counts': dict(refined_reason_counts), 'partial_publish_summary': summary,
    }
    (SHORT / 'index.json').write_text(json.dumps(index, ensure_ascii=False, indent=2), encoding='utf-8')
...
def main():
    ...
    print('Steam collector accelerator:', json.dumps(accelerator._stats, ensure_ascii=False, sort_keys=True))
