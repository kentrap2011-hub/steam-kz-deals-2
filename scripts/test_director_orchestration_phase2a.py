...
class Phase2ASecurityTests(unittest.TestCase):
    ...
    def test_17_dispatch_disabled_yields_non_executable_staging_candidate(self):
        p=staging_plan(CONTRACT,copy.deepcopy(STATE),NOW); self.assertFalse(p['dispatch_enabled']); self.assertFalse(p['dispatch_performed']); self.assertFalse(p['openai_or_codex_invoked']); self.assertIsNotNone(p['candidate']); self.assertFalse(p['candidate']['executable']); self.assertIn(p['candidate']['mode'],{'READ_ONLY_RECON','AUDIT'})
